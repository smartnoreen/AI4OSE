use spinlock_lab::model::{Action, LockId, Task, TaskId};
use spinlock_lab::primitives::{LockKind, SleepBug, SpinBug};
use spinlock_lab::sim::{RunError, Sim};

#[test]
fn control_spin_unlock_does_not_release_times_out() {
    let lock = LockKind::Spin(
        spinlock_lab::primitives::SpinLock::new(LockId(0)).with_bug(SpinBug {
...
fn control_sleep_no_wakeup_deadlocks() {
    let lock = LockKind::Sleep(
        spinlock_lab::primitives::SleepLock::new(LockId(0)).with_bug(SleepBug {
            no_wakeup: true,
