use spinlock_lab::primitives::LockKind;
use spinlock_lab::sim::{run_experiment, ExperimentConfig};
...
fn stress_correct_versions_pass_and_are_observable() {
    ...
    let spin = run_experiment(cfg, LockKind::Spin(spinlock_lab::primitives::SpinLock::new(
        spinlock_lab::LockId(0),
    )))
    .unwrap();

    let sleep = run_experiment(cfg, LockKind::Sleep(spinlock_lab::primitives::SleepLock::new(
        spinlock_lab::LockId(0),
    )))
    .unwrap();
