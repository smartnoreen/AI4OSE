use semaphore_lab::primitives::SemaphoreKind;
use semaphore_lab::sim::{run_experiment, ExperimentConfig};
...
fn stress_correct_versions_pass_and_are_observable() {
    ...
    let spin = run_experiment(
        cfg,
        SemaphoreKind::Spin(semaphore_lab::primitives::SpinSemaphore::new(
            semaphore_lab::SemId(0),
            1,
        )),
    )
    .unwrap();
    ...
    let sleep = run_experiment(
        cfg,
        SemaphoreKind::Sleep(semaphore_lab::primitives::SleepSemaphore::new(
            semaphore_lab::SemId(0),
            1,
        )),
    )
...
#[test]
fn sleep_semaphore_has_fewer_context_switches_in_high_contention() {
    let cfg = ExperimentConfig {
        num_tasks: 8,
        iterations: 20,
        permits: 1,
