...
impl MemorySet {
    ...
    /// mmap: map a new area with given start address, length and permission
    pub fn mmap(&mut self, start: usize, len: usize, prot: usize) -> isize {
        ...
        // Check if prot is valid (bits 0-2 are valid, bit 3+ are invalid)
        if prot & !0x7 != 0 {
            return -1;
        }
        // Check if prot has at least one permission bit set
        if prot & 0x7 == 0 {
            return -1;
        }
        ...
        // Safety check: ensure the range is reasonable (less than 1GB)
        if len > 0x40000000 {
            error!("mmap: len too large: {:#x}", len);
            return -1;
        }
    ...
    /// munmap: unmap an area with given start address and length
    pub fn munmap(&mut self, start: usize, len: usize) -> isize {
        ...
        let mut area = self.areas.remove(area_idx.unwrap());
